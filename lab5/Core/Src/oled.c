...
//	/*STM32F103C8T6???????I2C: PB6 -- SCL; PB7 -- SDA */
...
//	I2C_InitStructure.I2C_OwnAddress1 = 0x30;//??????I2C???,??????
...
void WriteCmd(unsigned char I2C_Command)//?????     &hi2c1
{
//    uint8_t pData[1];
//    pData[0] = I2C_Command;
    uint8_t *pData;
...
void OLED_Init(void)
{
	...
	//WriteCmd(0xc8);	//COM?????????COM[N-1]??COM0????????????????????
	WriteCmd(0xc0);	//COM?????????COM0??COM[N-1]??????????????
	WriteCmd(0x00); //---set low column address
	WriteCmd(0x10); //---set high column address
	WriteCmd(0x40); //--set start line address
	WriteCmd(0x81); //--set contrast control register
	WriteCmd(0xff); //??????? 0x00~0xff
	//WriteCmd(0xa1); //????127???SEG0??????????????
	WriteCmd(0xa0);  //????0???SEG0??????????
